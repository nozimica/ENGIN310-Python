...
\newcommand{\drawBranch}[2]{%
    ...
    {%
        ...
        {%
            ...
            {
                \node[] (f#1-ret) {\the\cnt~~*}; \& \node[fill=gray!40] (f#1) {\texttt{factorial(#1)}}; \\
            };
        }{%
        ...
        \structure<\showat>{%
            \draw[signLine] (f\the\cnt) to[out=-50, in=30] node [right] {Paso \step: ejecuta \texttt{factorial(\the\cnt)}} ($(f#1-global)+(.05,.5)$);
        }
    }{%
...
\begin{document}
\begin{frame}{Cálculo recursivo del Factorial, paso a paso}
