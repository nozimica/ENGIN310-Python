...
\begin{enumerate}
    \footnotesize
    \item \parbox{23mm}{(a, $\alpha$, $\beta$)} $ A = a^2 * sen(\alpha) = a^2 * sen(\beta) $
    \item \parbox{23mm}{(a, r)} $ A = 2a * r $
    \item \alert<2->{\parbox{23mm}{(p, q)} $ A = \frac{p * q}{2} $}
    \item \parbox{23mm}{Coordenadas} $ A = \left( \frac{\sqrt{(x_c - x_a)^2 + (y_c - y_a)^2} * \sqrt{(x_d - x_b)^2 + (y_d - y_b)^2}}{2} \right) $
\end{enumerate}
